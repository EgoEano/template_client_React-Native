    ...
    return (
        <Routes>
            ...
            <Route
                path="*"
                element={
                    <PlaceholderScreen name="Where did you find this page?" />
                }
            />
        </Routes>
    );
...
function renderNode(node: RouteNode): React.ReactNode {
    ...
    if (Array.isArray(node.children) && node.children.length > 0) {
        return (
            <Route
                ...
                element={
                    node.component ? (
                        <LazyWrapper component={node.component} />
                    ) : undefined
                }
            >
...
export function PagesTabRootNavigatorRouter({
    ...
    const DefaultComponent =
        Default ?? memoizedRoutes[0]?.component ?? (() => <></>);
    ...
    return (
        <Routes>
            ...
            {memoizedRoutes?.map((r) => {
                ...
                const paths = Array.isArray(pathSelect)
                    ? pathSelect
                    : [pathSelect];
